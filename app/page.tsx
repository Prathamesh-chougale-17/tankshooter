...
export default function HomePage() {
  ...
  const payGasFee = async () => {
    ...
    const result = await payGameGasFee(account.address, cluster?.label);
        ...
        <Card className="w-full max-w-2xl bg-black/30 backdrop-blur-xl border-purple-500/30 shadow-2xl shadow-purple-500/10 relative overflow-hidden group animate-slide-up">
          ...
          <CardContent className="space-y-8 relative z-10">
            ...
            >
              ...
              <div className="flex items-center gap-3 p-4 bg-white/5 border border-white/10 rounded-lg backdrop-blur-sm relative z-50">
                <Wallet
                  className={`h-5 w-5 ${
                    account?.address ? "text-green-400" : "text-yellow-400"
                  }`}
                />
                <div className="flex-1">
                  {account?.address ? (
                    ...
                    <div>
                      ...
                      <div className="text-gray-400 text-xs">
                        Connect wallet to pay gas fee (0.0001 SOL)
                      </div>
                    </div>
                  )}
                </div>
            ...
            >
              ...
              {!account?.address && (
                <p className="text-center text-gray-400 text-sm mt-3">
                  Connect your Backpack wallet and pay 0.0001 $GOR to play on
                  Gorbagana testnet
                </p>
              )}
            </div>
