...
export async function POST(request: NextRequest): Promise<NextResponse<ApiResponse>> {
  ...
  try {
    ...
    if (debugMode) {
      debugInfo.request = { playerName, playerWallet, prizeAmount, competitionId };
      console.log('🐛 [DEBUG] Processing native GOR transfer:', debugInfo.request);
    }
    ...
    if (debugMode) {
      ...
      console.log('🐛 [DEBUG] Native GOR transfer details:', debugInfo.tokenTransfer);
    }
    ...
    if (balance < amountInLamports + transactionFee) {
      return NextResponse.json(
        { 
          success: false, 
          error: `Insufficient GOR balance. Need ${(amountInLamports + transactionFee) / LAMPORTS_PER_SOL} GOR, have ${balanceInGOR} GOR`,
    ...
    try {
      ...
      if (errorMsg.includes('insufficient')) {
        ...
        troubleshooting = 'Check GOR balance and ensure enough for transaction fees.';
...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    return NextResponse.json({
      ...
      networkInfo: {
        totalSupply: (supply.value.total / LAMPORTS_PER_SOL).toFixed(2) + ' GOR',
        circulatingSupply: (supply.value.circulating / LAMPORTS_PER_SOL).toFixed(2) + ' GOR',
