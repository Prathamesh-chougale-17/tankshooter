...
function getTankColor(tankClass: string): string {
  const colors = {
    basic: "#4A90E2",
    twin: "#E24A4A",
    sniper: "#4AE24A",
...
wss.on("connection", (ws: WebSocket) => {
  ...
  const activeGames = Array.from(gameRooms.values()).filter(
    (room) => room.tanks.size > 0
  ).length;
  ws.send(
    JSON.stringify({
      type: "serverStats",
      ...
      switch (message.type) {
        ...
        case "playerUpdate":
          ...
          if (room && room.tanks.has(playerId)) {
            const tank = room.tanks.get(playerId)!;
            // Update tank data
            if (message.position) tank.position = message.position;
            if (message.rotation !== undefined)
              tank.rotation = message.rotation;
            if (message.health !== undefined) tank.health = message.health;
            if (message.score !== undefined) tank.score = message.score;
            if (message.level !== undefined) tank.level = message.level;
            if (message.kills !== undefined) tank.kills = message.kills;
...
server.listen(PORT, () => {
  console.log(
    `🎮 Tank Shooter Multiplayer Server running on http://localhost:${PORT}`
  );
