  ...
  const leaderboardData: LeaderboardEntry[] = gameData
    ...
    : // Fallback static data
      [
        ...
        { rank: 3, name: "DiepLord", score: 32150, level: 38, kills: 16 },
        { rank: 4, name: "You", score: 15420, level: 25, kills: 8, isCurrentPlayer: true },
        ...
        {leaderboardData.map((player) => (
          <div
            ...
            className={`flex items-center justify-between p-2 rounded ${
              player.isCurrentPlayer ? "bg-purple-600/20 border border-purple-500/30" : "bg-white/5 hover:bg-white/10"
            }`}
          >
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-1">
                {getRankIcon(player.rank)}
                <span className={`text-xs font-bold ${getRankColor(player.rank)}`}>#{player.rank}</span>
              </div>

              <div>
                <div
                  className={`text-xs font-medium flex items-center gap-1 ${player.isCurrentPlayer ? "text-purple-300" : "text-white"}`}
                >
                  {player.name}
