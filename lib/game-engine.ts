...
class AIBot {
  ...
  private updateMovement(deltaTime: number) {
    ...
    if (Date.now() > this.bot.changeDirectionTime || this.shouldAvoidBoundary()) {
  ...
  private shouldAvoidBoundary(): boolean {
    const boundary = 1500
    return Math.abs(this.bot.position.x) > boundary || Math.abs(this.bot.position.y) > boundary
  }
  ...
  private updateShooting(deltaTime: number) {
    ...
    if (this.bot.target && this.isInShootRange() && now - this.bot.lastShot > shootCooldown) {
      this.shoot()
  ...
  private isInAggroRange(): boolean {
    ...
    return this.getDistance(this.bot.position, this.bot.target) <= this.bot.aggroRange
  }

  private isInShootRange(): boolean {
    if (!this.bot.target) return false
    return this.getDistance(this.bot.position, this.bot.target) <= this.bot.shootRange
  }
...
export class GameEngine {
  ...
  private spawnBot(difficulty: "easy" | "medium" | "hard" = "medium") {
    ...
    const spawnX = (player?.position.x || 0) + Math.cos(spawnAngle) * spawnDistance
    const spawnY = (player?.position.y || 0) + Math.sin(spawnAngle) * spawnDistance
    ...
    const bot: Bot = {
      ...
      maxHealth: difficulty === "easy" ? 200 : difficulty === "medium" ? 300 : 400,
      ...
      aggroRange: difficulty === "easy" ? 350 : difficulty === "medium" ? 450 : 600,
      shootRange: difficulty === "easy" ? 300 : difficulty === "medium" ? 400 : 550,
      difficulty,
    }
    ...
    this.gameState.bullets = this.gameState.bullets.filter((bullet) => {
      for (const [tankId, tank] of this.gameState.tanks) {
        ...
        const distance = Math.sqrt(
          Math.pow(bullet.position.x - tank.position.x, 2) + Math.pow(bullet.position.y - tank.position.y, 2),
        )
  ...
  private spawnBotsIfNeeded() {
    ...
    if (now - this.botSpawnTimer > this.botSpawnInterval && this.bots.size < this.maxBots) {
      const difficulty = this.getDynamicDifficulty()
      this.spawnBot(difficulty)
  ...
  private getDynamicDifficulty(): "easy" | "medium" | "hard" {
    ...
    if (scoreThreshold > 20 || levelThreshold > 15) {
      ...
      return Math.random() < 0.3 ? "hard" : Math.random() < 0.6 ? "medium" : "easy"
    } else {
  ...
  private updatePlayer(deltaTime: number) {
    ...
    player.rotation = Math.atan2(this.mouse.y - centerY, this.mouse.x - centerX)

    // Health regeneration - only when not shooting
    const now = Date.now()
    const timeSinceLastShot = now - this.lastPlayerShot

    if (player.health < player.maxHealth && timeSinceLastShot > this.healthRegenCooldown) {
  ...
  private drawGameOverOverlay() {
    ...
    this.ctx.fillText("Your tank has been destroyed!", this.canvas.width / 2, this.canvas.height / 2 - 40)
  ...
  private drawTank(tank: Tank) {
    ...
    this.ctx.fillRect(tank.position.x - barWidth / 2, tank.position.y + size / 2 + 5, barWidth, barHeight)
